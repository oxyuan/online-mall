...
import cn.boyce.manager.repo.ItemParamDao;
...
public class ItemParamServiceImpl implements ItemParamService {
    ...
    public R addItemParam(Long cid, String paramData) {
        ItemParam itemParam = new ItemParam();  // 此时获取 itemParam 的 id 值为 null
        itemParam.setItemCatId(cid);
        itemParam.setParamData(paramData);
        itemParam.setCreated(new Date());
        itemParam.setUpdated(new Date());
        itemParamDao.saveAndFlush(itemParam);
        Long result = itemParam.getId();   // 此时获取 itemParam 的 id 值为 数据库自增后的值
        if (result != null) {
            return R.ok();
        } else {
